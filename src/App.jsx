...
function SidebarConfig({ tenants, setTenants, schedule, setSchedule, delta, setDelta,
    ghToken, setGhToken, ghRepo, setGhRepo, geminiKey, setGeminiKey, entityProfile, setEntityProfile, onDeploy }) {
          ...
          <div style={{ padding:"6px 20px 10px", display:"flex", gap:6, flexWrap:"wrap",
            ...
            {flowSteps.some(s=>!s.lt||s.lt==="UNKNOWN") && (
              <span style={{ ...mono, fontSize:7, color:"#f59e0b" }}>
                + {flowSteps.filter(s=>!s.lt||s.lt==="UNKNOWN").length} unmapped sourcetypes (skipped in workflow)
              </span>
            )}
          </div>
        </div>
...
${flowSteps.filter(s => s.lt && s.lt !== "UNKNOWN").map((s,i) => {
...
def ingest_entities_direct(ndjson_file, credentials_path, customer_id, region, log_type):
    """
    ...
    # Region -> malachiteingestion-pa host (matches logstory's LegacyIngestionBackend)
    ...
    prefix = prefix_map.get(region_upper, "")
    url = f"https://{prefix}malachiteingestion-pa.googleapis.com/v2/entities:batchCreate"
    print(f"[info] Entity API: {url}")
    ...
    <div style={{ display:"flex", height:"100vh", overflow:"hidden",
      ...
      <div style={{ width:280, flexShrink:0, borderRight:"1px solid #08172c",
        ...
        <SidebarConfig
          ...
          entityProfile={entityProfile} setEntityProfile={setEntityProfile}
